...
function NavLinks({ items }: { items: typeof navItems }) {
    ...
    <header className="sticky top-0 z-50 mx-auto w-full max-w-5xl px-3 py-2.5 sm:px-5 sm:py-3">
      <nav className="glass-strong flex items-center justify-between rounded-[1.25rem] px-4 py-2.5 sm:rounded-[1.5rem] sm:px-5">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          {/* Nav links: desktop only — BottomTabNav handles mobile */}
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-5xl px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:hidden">
      <div className="glass-strong flex items-center justify-around rounded-[1.25rem] px-2 py-1.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("text-[9px] font-medium", active && "text-cyan-400")}>
                {item.label}
              </span>
            </Link>
          )
        ...
        <button
          onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
          className="flex flex-col items-center gap-0.5 rounded-xl px-3 py-1.5 text-muted-foreground transition-colors hover:text-foreground hover:bg-muted/30"
          aria-label="Toggle theme"
        >
